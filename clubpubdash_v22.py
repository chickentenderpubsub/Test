...
with st.sidebar.expander("Analysis Settings", expanded=False):
    trend_analysis_weeks = st.slider("Trend Window (Weeks)", 3, 8, 4, key="trend_window", 
                                    help="Weeks for category trend calculation.")
    z_threshold = st.slider("Anomaly Sensitivity (Z)", 1.0, 3.0, 2.0, 0.1, key="anomaly_z", 
...
if display_week is not None:
    ...
    perf_data = df_filtered  # Use overall filtered if no specific week
...
if not all(col in df_filtered.columns for col in ['Store #', 'Week', 'Engaged Transaction %']):
    ...
    if trend_corr_data:
        ...
        store_stats = store_stats_agg
        store_stats['Trend Correlation'] = 0.0
    ...
    if not store_stats.empty:
        ...
        def assign_category_original(row):
            ...
            is_above = avg_eng >= median_engagement
            pos_trend = trend_corr > trend_threshold
            neg_trend = trend_corr < -trend_threshold
            
            if is_above:
                return "Needs Stabilization" if neg_trend else "Star Performer"
            else:
                return "Improving" if pos_trend else "Requires Intervention"
...
with tab_overview:
    ...
    with col1:
        ...
        metric_suffix = "(Period Avg)"
        
        if store_choice:
            metric_label = f"Avg ({len(store_choice)} Stores)" if len(store_choice) > 1 else f"Store {store_choice[0]}"
        
        if display_week is not None:
            metric_suffix = f"(Wk {int(display_week)})"
    ...
    if 'Store #' not in df_filtered.columns or 'Engaged Transaction %' not in df_filtered.columns:
        ...
        if not comp_data.empty:
            ...
            bar_chart = alt.Chart(comp_data).mark_bar().encode(
                ...
                tooltip=[
                    alt.Tooltip('Store #:N'),
                    alt.Tooltip('Engaged Transaction %:Q', format='.1f')
                ]
            )
            ...
            rule = alt.Chart(pd.DataFrame({'avg': [comparison_avg]})).mark_rule(
                ...
                x=alt.X('avg:Q'),
                tooltip=[alt.Tooltip('avg:Q', title='Average', format='.1f')]
            )
            ...
            if not rank_data.empty and pd.api.types.is_numeric_dtype(rank_data['Weekly Rank']):
                ...
                rank_chart = alt.Chart(rank_data).mark_line(point=True).encode(
                    ...
                    y=alt.Y(
                        ...
                        scale=alt.Scale(domain=[max_r + 1, min_r - 1 if min_r > 0 else 0])  # Inverted
                    ),
                    color=alt.Color('Store #:N', legend=None),
                    tooltip=[
                        alt.Tooltip('Store #:N'),
                        alt.Tooltip('Week:O'),
                        alt.Tooltip('Weekly Rank:Q', title='Rank')
                    ]
                ...
                st.info("Weekly rank data not available/numeric for the selected filters.")
...
with tab_categories:
    ...
    if store_stats.empty:  # Check if category calculation failed
        ...
        category_definitions = {  # Using original categories
            "Star Performer": {
                "icon": "⭐",
                "desc": "High engagement with stable/positive trend.",
                "color": CAT_COLORS["Star Performer"]
            },
            "Needs Stabilization": {
                "icon": "⚠️",
                "desc": "High engagement but negative trend.",
                "color": CAT_COLORS["Needs Stabilization"]
            },
            "Improving": {
                "icon": "📈",
                "desc": "Below median but positive trend.",
                "color": CAT_COLORS["Improving"]
            },
            "Requires Intervention": {
                "icon": "🚨",
                "desc": "Below median with flat/negative trend.",
                "color": CAT_COLORS["Requires Intervention"]
            }
        }
        ...
        for i, (cat, info) in enumerate(defs_list):
            ...
            with target_col:
                st.markdown(
                    f"""<div class="metric-card" style="border-left: 5px solid {info['color']};">
        ...
        for i, cat in enumerate(category_order):
            ...
            with cols_cat_disp[i]:
                st.markdown(
                    f"""<h4 style='color:{info['color']}; border-bottom: 2px solid {info['color']}; 
                    padding-bottom: 5px; margin-bottom: 10px;'>{info['icon']} {cat} ({len(cat_stores)})</h4>""",
                    unsafe_allow_html=True
                )
                
                if not cat_stores.empty:
                    cat_stores_sorted = cat_stores.sort_values(
                        'Average Engagement', 
                        ascending=(cat not in ["Star Performer", "Needs Stabilization"])
                    )
                    for idx, store_row in cat_stores_sorted.iterrows():
                        ...
                        st.markdown(
                            f"**Store {store_row['Store #']}** <small>({store_row['Average Engagement']:.1f}%)</small><br>{trend_info}",
                            unsafe_allow_html=True
                        )
                else:
                    st.caption("None")
...
with tab_anomalies:
    ...
    if anomalies_found.empty:
        ...
        if 'Rank' in anomalies_found.columns and 'Prev Rank' in anomalies_found.columns and not anomalies_found[['Rank', 'Prev Rank']].isna().all():
...
st.sidebar.caption(f"© Publix Supermarkets, Inc. {datetime.date.today().year}")
    ...
    with col2:
        ...
        if pd.notna(donut_value):
            st.altair_chart(create_donut_chart(donut_value, title="Engaged"), use_container_width=True)
        else:
            st.text("N/A")
    ...
    if 'Week' not in df_filtered.columns or 'Engaged Transaction %' not in df_filtered.columns:
        st.warning("Missing data for trend chart.")
    else:
        ...
        lines = base_chart.mark_line(point=False).encode(
            ...
            color=alt.Color(
                'Store #:N',
                legend=alt.Legend(title="Store", orient="top-left") if store_choice and 0 < len(store_choice) <= 10 else None
            ),
            tooltip=[
                alt.Tooltip('Store #:N'),
                alt.Tooltip('Week:O'),
        ...
        if (not store_choice or len(store_choice) > 1) and not avg_trend_data_filtered.empty:
            district_avg_line = alt.Chart(avg_trend_data_filtered).mark_line(
                ...
                tooltip=[
                    alt.Tooltip('Week:O'),
                    alt.Tooltip('Engaged Transaction %:Q', format='.1f', title='District Avg')
                ]
            )
        ...
        if (not store_choice or len(store_choice) > 1) and not avg_trend_data_filtered.empty:
            st.caption("Dashed line indicates average across selection.")
